...
class LLM_Service:
    def __init__(self):
        ...
        def retrieve_papers(query: str) -> str:
            ...
            retrieved_docs = self.llm_gemini.invoke("Summarize this documents" + "\n".join([doc.page_content for doc in retrieved_docs]))
        ...
        tools = [
            Tool(
                ...
                func=self.tavily_search_tool,  # Directly use the tavily_search_tool function
                description="Use Tavily to search for relevant information."
            )
        ]

        # Initialize the agent using the correct agent type
        nutritionist_agent = initialize_agent(
            tools=tools,
            llm=self.llm_gemini,
            agent_type=AgentType.ZERO_SHOT_REACT_DESCRIPTION,  # Adjust this based on your use case
            verbose=True,
        ...
        prompt_nutritionist = ChatPromptTemplate([
            ...
            HumanMessagePromptTemplate.from_template(
                ...
                "- Provide one concise scientific rationale for each nutrient. Use tavily_search_tool when necessary.\n"
                ...
                "  - `reason` (list of string): Scientific rationale for each nutrient, with APA citation and links.\n"
                "  - `minimum` (list integers): Minimum intake recommendation for each nutrient.\n"
                "\n"
                "**Output Format Example:**\n"
                '{{\n'
                '  "reason": ["More calories are needed due to energy expenditure","More protein is needed."]\n'
                '  "minimum": [2000, 1000]\n'
                '}}\n\n'
        ...
        #         cleaned_response = response['output'].strip("`").strip('json').strip('\n\n')
